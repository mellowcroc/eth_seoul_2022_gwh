...
import { Challenge__factory, Challenge, DonationFactory, DonationFactory__factory, Donation, Donation__factory } from "contracts";
...
export const useDonation = (donationFactoryAddress: string, walletAddress: string) => {
  ...
  const fetchDonations = useCallback(async () => {
    ...
    const donationFactory = new Contract(donationFactoryAddress, donationFactoryAbi, library) as DonationFactory;
    ...
    for (let i = 0; i < numDonations; i++) {
      ...
      const donation = new Contract(donationAddress, donationAbi, library) as Donation;
      ...
      if (donationData.challengesLength > 0) {
        ...
        const recentChallenge = new Contract(recentChallengeAddress, challengeAbi, library) as Challenge;
